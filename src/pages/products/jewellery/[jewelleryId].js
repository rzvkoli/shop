...
function JewelleryId({ product , data }) {
...
export const getStaticPaths = async () => {
    ...
    const paths = ids.map(id => ({ params:{ jewelleryId:id.toString() } }))
  ...
  export const getStaticProps = async ({ params }) => {
    const [productRes, allProductsRes] = await Promise.all([
      fetch(`https://fakestoreapi.com/products/${params.jewelleryId}`),
